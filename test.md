Grausami Welt, you are so very cruel
...
## Globals
Sometimes you might want to define a global variant variable, for example to set the
...
In these cases, the `extraConfigVim` and `extraConfigLua` options ar
In these cases, the `extraConfigVim` and `extraConfigLua` options ar
